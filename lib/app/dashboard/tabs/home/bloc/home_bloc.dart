...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc(this.repo) : super(HomeState()) {
    ...
    on<OnSelecteHospitalData>(_onSelecteHospitalId);
  }
  ...
  void _onGetAllHospital(
    ...
    try {
      ...
      ApiResponseHandler.handle<List<AllHospitalModel>, HomeState>(
        ...
        onError: (state, mess) =>
            state.copyWith(allHospitalSatatus: ApiStatus.error, mess: mess),
      );
    } catch (e) {
      emit(
        state.copyWith(mess: e.toString(), allHospitalSatatus: ApiStatus.error),
      );
    }
  }

  void _onSelecteHospitalId(
    OnSelecteHospitalData event,
    Emitter<HomeState> emit,
  ) => emit(state.copyWith(selectedHospital: event.data));
}
